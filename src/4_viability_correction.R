...
func_ = function(CORRECT, FITPLOT, HEATPLOT, inURL = '../data/', outURL = '../out/')
{
  ...
  if(CORRECT)
  {
    ...
    for(plate_ in plates_)
    {
      ...
      for(rw_ in 1:nrow(annot_tmp))
      {
        fcs_dt = read.FCS(filename = paste0(inURL,annot_tmp$file[rw_],'.fcs'), transformation = F, truncate_max_range = F)     # matrix of events in this well
        ...
        keyword(fcs_dt)[['$FIL']] = paste0(annot_tmp$file[rw_],'_compensated_corrected_corrected')
...
null_ = func_(CORRECT = CORRECT, FITPLOT = FITPLOT, HEATPLOT = HEATPLOT)
message('\nDone!\n')
summary(warnings())
