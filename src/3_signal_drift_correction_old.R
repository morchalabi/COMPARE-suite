...
if(FITPLOT)
{
  pdf(file = '../out/MFI_fit_intra_plates.pdf')
  for(plate_ in plates_)
  {
    for (chnl_ in chnls_)
    {
      y_ = matrix(data = MFI_mats[[plate_]][[chnl_]], ncol = 1, nrow = length(MFI_mats[[plate_]][[chnl_]]), byrow = F)/1e4
      x_ = 1:length(y_)                                                                                           # x_ then is offset of y_
      ...
      p_ = ggplot(data = data.frame(x = x_, y = y_, Plate = plate_), aes(x = x_, y = y_, color = as.factor(Plate)))+
        ...
        labs(x = 'Pseudo-time', y = expression('MFI ('%*%10^-4*')'), title = paste0('Intra-plate MFIs (',chnl_,')'), color = 'Plate')+
...
if(FITPLOT)
{
  ...
  for(plate_ in plates_)
  {
    for (chnl_ in chnls_)
    {
      tmp = matrix(data = MFI_mats[[plate_]][[chnl_]], ncol = 1, nrow = length(MFI_mats[[plate_]][[chnl_]]), byrow = F)/1e4
  ...
  pdf(file = '../out/MFI_fit_inter_plate.pdf')
  for(chnl_ in chnls_)
  {
